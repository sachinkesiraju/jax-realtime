...
- **Smaller download** — Pocket TTS and Whisper ship per-row int8 builds
  (104 MB instead of 201 MB and 73 MB instead of 144 MB) and are dequantized to
  fp16 during load. The SmolLM2 brain stays full fp16 for conversation quality.
...
Safari 26+). Click **Load models** (~943 MB on first run — SmolLM 724 +
Pocket TTS 104 + Whisper 73 + D-FINE 42, all cached in OPFS afterwards). The
optional Eye is off by default; enabling it adds the 42 MB D-FINE model and
requests camera access. Then press the orb once and just talk — hands-free: turn ends are
...
> SmolLM2-360M weights and a per-row int8 Pocket TTS decode checkpoint from
> [Hugging Face](https://huggingface.co/sachink98/jax-realtime-weights); the
> Whisper base.en weights are still fetched as a per-row int8 build. All
> quantized artifacts are dequantized to fp16 during load. Whisper produced
> identical paired MAP and holdout transcripts; the SmolLM2 brain is kept at full
> precision for the best conversation quality.
