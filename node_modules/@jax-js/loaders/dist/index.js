...
var SentencePiece = class SentencePiece {
	...
	constructor(model) {
		this.#modelType = model.trainerSpec?.modelType ?? TrainerSpec_ModelType.UNIGRAM;
		if (this.#modelType !== TrainerSpec_ModelType.UNIGRAM && this.#modelType !== TrainerSpec_ModelType.BPE) throw new Error(`Unsupported SentencePiece model type: ${this.#modelType}`);
		...
		if (this.#modelType === TrainerSpec_ModelType.UNIGRAM) this.#trie = createTrieNode();
		for (let i = 0; i < model.pieces.length; i++) {
			...
			} else if (type === ModelProto_SentencePiece_Type.USER_DEFINED) this.#specialTokens.set(pieceStr, i);
			if (this.#modelType === TrainerSpec_ModelType.UNIGRAM && (type === ModelProto_SentencePiece_Type.NORMAL || type === ModelProto_SentencePiece_Type.USER_DEFINED)) this.#insertIntoTrie(pieceStr, i, score);
		}
	}
	get modelType() {
		return this.#modelType === TrainerSpec_ModelType.UNIGRAM ? "unigram" : "bpe";
	}
	static fromBinary(data) {
		return new SentencePiece(fromBinary(ModelProtoSchema, data));
	}
	...
	*/
	...
	encode(text) {
		if (this.#modelType === TrainerSpec_ModelType.BPE) return this.#encodeWithSpecialTokens(text, (fragment) => this.#encodeBpeFragment(fragment));
...
//#endregion
export { OPFS, WeightMapper, cachedFetch, opfs, safetensors_exports as safetensors, tokenizers_exports as tokenizers };
